...
class tiktok:
    ...
    def update_title(self):
        ...
        ctypes.windll.kernel32.SetConsoleTitleW(
            f"TikTok Username Checker | Kullanılmıyor: {self.available} | Kullanılıyor: {self.unavailable} | Yapılan: {(self.available + self.unavailable)} | Kalan: {remaining} | Developed by Voxi"
        )
    ...
    def check_username(self, username):
        with requests.Session() as session:
            ...
            if r.status_code == 200:
                self.unavailable += 1
                self.print_console("Kullanılıyor", username)
            elif r.status_code == 404:
                self.available += 1
                self.print_console("Kullanılmıyor", username, Fore.GREEN)
    ...
    def main(self):
        os.system("cls")
        ctypes.windll.kernel32.SetConsoleTitleW("TikTok Username Checker | Developed by Voxi")
